...
TEST(PRELU, xnn_operator_lifetime) {
  ...
  // One new xnn_operator should be created for the first call to elu.
  tfjs::wasm::Elu(x0_id, weights0_id, out_id);
  ASSERT_EQ(1, tfjs::backend::xnn_operator_count);

  // No new xnn_operators should be created for the second call to elu with
  // the same weights.
  tfjs::wasm::Elu(x1_id, weights0_id, out_id);
  ASSERT_EQ(1, tfjs::backend::xnn_operator_count);

  // One new xnn_operator should be created for another call to elu with new
  // weights.
  tfjs::wasm::Elu(x0_id, weights1_id, out_id);
  ASSERT_EQ(2, tfjs::backend::xnn_operator_count);

  // No new xnn_operators should be created for the next call to elu with
  // the same weights.
  tfjs::wasm::Elu(x1_id, weights1_id, out_id);
  ASSERT_EQ(2, tfjs::backend::xnn_operator_count);
