...
void delete_xnn_operator(const size_t weights_id) {
  xnn_operator_t elu_op = operator_cache.at(weights_id);
  xnn_delete_operator(elu_op);
...
void elu(const float* x_buf, const size_t x_size, const size_t weights_id,
         const size_t out_id) {
  ...
  if (operator_cache_idx == operator_cache.end()) {
    ...
    xnn_status status = xnn_create_elu_nc_f32(channels, strides, strides,
                                              *weights_buf, flags, &elu_op);

    if (status != xnn_status_success) {
      util::warn(
          "XNN status for xnn_create_elu_nc_f32 is not successful. Got "
          "status %d. Use -c dbg to see XNN logs.",
          status);
    }

    operator_cache.insert({weights_id, elu_op});
  ...
  xnn_status status = xnn_setup_elu_nc_f32(elu_op, batch_size, x_buf, out_buf,
                                           tfjs::backend::threadpool);
  if (status != xnn_status_success) {
    util::warn(
        "XNN status for xnn_setup_elu_nc_f32 is not successful. Got "
        "status %d. Use -c dbg to see XNN logs.",
        status);
  }
